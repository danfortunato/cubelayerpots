...
C
     ...
     +              FARGS,EPSABS,EPSREL,KEY,RESTAR,NUM,LENW,WTLENG,
...
C            evaluation point.
C            It must have parameters (NDIM,X,NUMFUN,FARGS,FUNVLS)
C            Input parameters:
...
C
     ...
     +        FUNSUB,FARGS,SCALES,NORMS,X,WORK,VALUES(1,1),ERRORS(1,1),
     +            DIR(1))
      NEVAL = NEVAL + NUM
C
...
Cvd$l cncall
     ...
     +                    G(1,1,I),W,ERRCOF,NUMFUN,FUNSUB,FARGS,SCALES,
     +                    NORMS,X(1,I),WORK(L1),VALUES(1,INDEX),
...
C
      END
      SUBROUTINE DCUHRE(NDIM,NUMFUN,A,B,MINPTS,MAXPTS,FUNSUB,FARGS,
     +          EPSABS,EPSREL,KEY,NW,RESTAR,RESULT,ABSERR,NEVAL,IFAIL,
     +                  WORK)
...
C
      LENW = 16*MDIV*NUMFUN
      CALL DADHRE(NDIM,NUMFUN,MDIV,A,B,MINSUB,MAXSUB,FUNSUB,FARGS,
     +            EPSABS,EPSREL,KEYF,RESTAR,NUM,LENW,WTLENG,
...
C
      END
      SUBROUTINE DFSHRE(NDIM,CENTER,HWIDTH,X,G,NUMFUN,FUNSUB,FARGS,
     +                  FULSMS,FUNVLS)
...
30    CONTINUE
40    CALL FUNSUB(NDIM,X,NUMFUN,FARGS,FUNVLS)
      DO 50 J = 1,NUMFUN
...
C
     ...
     +                  FUNSUB,FARGS,SCALES,NORMS,X,NULL,BASVAL,
     +                  RGNERR,DIRECT)
...
10    CONTINUE
      CALL FUNSUB(NDIM,X,NUMFUN,FARGS,RGNERR)
      DO 30 J = 1,NUMFUN
...
30    CONTINUE
      ...
      DO 60 I = 1,NDIM
          ...
          CALL FUNSUB(NDIM,X,NUMFUN,FARGS,NULL(1,5))
          X(I) = CENTER(I) + HWIDTH(I)*G(1,2)
          CALL FUNSUB(NDIM,X,NUMFUN,FARGS,NULL(1,6))
          X(I) = CENTER(I) - HWIDTH(I)*G(1,3)
          CALL FUNSUB(NDIM,X,NUMFUN,FARGS,NULL(1,7))
          X(I) = CENTER(I) + HWIDTH(I)*G(1,3)
          CALL FUNSUB(NDIM,X,NUMFUN,FARGS,NULL(1,8))
          X(I) = CENTER(I)
          DIFSUM = 0
          DO 50 J = 1,NUMFUN
...
C
      DO 90 I = 4,WTLENG
          CALL DFSHRE(NDIM,CENTER,HWIDTH,X,G(1,I),NUMFUN,FUNSUB,FARGS,
     +                RGNERR,NULL(1,5))
          DO 80 J = 1,NUMFUN
