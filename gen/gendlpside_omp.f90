      subroutine gendlpside(rlam, nnodes, tabdlpside)
!***********************************************************************
!
! This subroutine generates quadratures for DLP due to polynomial basis
...
!***********************************************************************
      ...
      real *8 xnodes(100)
      real *8 weights(100)
      real *8 tabdlpside(nnodes*nnodes,nnodes*nnodes,6,6,-1:1)
      real *8 a(ndim), b(ndim)

!     Valid face pair table
      integer pairs(2,24)
...
!$OMP   SHARED(tabdlpside, x, umat, nnodes, nf, rlam, pairs, npairs, &
...
!        Decode flat index into (ipair, istep, npoint)
         ...
         call dcuhre(ndim,nf,a,b,mincls,maxcls,fdlpside,              &
...
!        polynomial coefficients.
         ...
         do n1 = 1,nnodes
            n = (n2-1)*nnodes+n1
            tabdlpside(npoint,n,ifacet,ifaces,istep) =                &
              chebtab2(n1,n2)
         enddo
         enddo
      enddo
...
!$OMP END PARALLEL
      ...
      subroutine fdlpside(ndim, z, nfun, fargs, f)
      implicit none
      integer ndim, nfun,nnodes,inext,j,k,ifaces
      real *8 z(ndim), f(nfun), rx, ry, rz, pi,rr2
      real *8 rr,reps,dgreen,dgreendz,drrdz,dlpg
      real *8 tx(100), ty(100)
      real *8 xtarg,ytarg,ztarg,fargs(6),rlam
      ...
      if (rr.gt.reps) then
         if (ifaces.eq.1) then
            dlpg =                                                      &
              -rx*exp(-rlam*rr)/(rr*rr2)-(rlam*rx*exp(-rlam*rr))/rr2
            dlpg = dlpg/(4.0d0*pi)
         else if (ifaces.eq.2) then
            dlpg =                                                      &
              rx*exp(-rlam*rr)/(rr*rr2)+(rlam*rx*exp(-rlam*rr))/rr2
            ...
            dlpg =                                                      &
              -ry*exp(-rlam*rr)/(rr*rr2)-(rlam*ry*exp(-rlam*rr))/rr2
            dlpg = dlpg/(4.0d0*pi)
         else if (ifaces.eq.4) then
            dlpg =                                                      &
              ry*exp(-rlam*rr)/(rr*rr2)+(rlam*ry*exp(-rlam*rr))/rr2
            ...
            dlpg =                                                      &
              -rz*exp(-rlam*rr)/(rr*rr2)-(rlam*rz*exp(-rlam*rr))/rr2
            dlpg = dlpg/(4.0d0*pi)
         else if (ifaces.eq.6) then
            dlpg =                                                      &
              rz*exp(-rlam*rr)/(rr*rr2)+(rlam*rz*exp(-rlam*rr))/rr2
